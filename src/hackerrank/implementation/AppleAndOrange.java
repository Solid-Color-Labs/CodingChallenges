...
public class AppleAndOrange {
    ...
    public static void main(String[] args) {
        ...
        int m = in.nextInt();
        int n = in.nextInt();
        int numberOfApplesOnHouse = calculateNumberOfFruitOnHouse(in, s, t, a, m);
        int numberOfOrangesOnHouse = calculateNumberOfFruitOnHouse(in, s, t, b, n);
        System.out.println(numberOfApplesOnHouse + "\n" + numberOfOrangesOnHouse);
    }
    
    private static int calculateNumberOfFruitOnHouse(Scanner in, int houseStart, int houseEnd, int treePosition, int numberOfFruits) {
        int numberOfFruitsOnHouse = 0;
        for(int i = 0; i < numberOfFruits; i++){
            int fruitDistanceFromTree = in.nextInt();
            int fruitPosition = fruitDistanceFromTree + treePosition;
            if (fruitPosition >= houseStart && fruitPosition <= houseEnd) {
                numberOfFruitsOnHouse++;
            }
        }
        return numberOfFruitsOnHouse;
    }
